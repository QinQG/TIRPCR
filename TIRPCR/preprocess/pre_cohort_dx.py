...
def get_user_dx(indir, patient_list, icd9_to_css, icd10_to_css):
    ...
    for file in files:
        ...
        for index, row in tqdm(inpat.iterrows(), total=len(inpat)):
            ...
            enrolid = row['ENROLID']
            date = row[DATE_NAME]
            DXVER = row.get('DXVER', '0').strip().split('.')[0]  # Clean and extract integer part
...
def pre_user_cohort_dx(user_dx, text_cad_prescription_taken_by_patient, min_patients):
    user_cohort_dx = AutoVivification()
    for drug, taken_by_patient in tqdm(text_cad_prescription_taken_by_patient.items()):
            ...
            for patient, taken_times in taken_by_patient.items():
                ...
                if date_codes:  
                    for date, codes in date_codes.items():
                        ...
                        if date < (index_date + timedelta(days=1)):
                    ...
                    print(f"No diagnosis codes found for patient {patient} under drug {drug}.")
...
def get_user_cohort_dx(indir, text_cad_prescription_taken_by_patient, icd9_to_css, icd10_to_css, min_patient, patient_list):
    user_dx = get_user_dx(indir, patient_list, icd9_to_css, icd10_to_css)
    return pre_user_cohort_dx(user_dx, text_cad_prescription_taken_by_patient, min_patient)
