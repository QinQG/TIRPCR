...
def pre_user_cohort_triplet(cad_prescription_taken_by_patient, cad_user_cohort_rx, cad_user_cohort_dx,
                            save_cohort_outcome,cad_user_cohort_demo, out_file_root):
    if not cad_user_cohort_rx:
...
def get_outcome_feature_vector(dates, index_date):
    for date in dates:
        if date >= index_date and (date - index_date).days <= 10:
            return 1
    return 0
